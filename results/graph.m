...
for i = 1:rows(m)
  if (exist(strcat(m(i,:),"_cat_hour4.csv")) == 2)
    clf
    clc
    model = m(i,:);
    filename = model;
...
for i = 1:rows(m)
  if (exist(strcat(m(i,:),"_S_hour4.csv")) == 2)
    clf
    clc
    model = m(i,:);
    filename = model;
...
for i = 1:rows(m)
  if (exist(strcat(m(i,:),"_SO3_hour4.csv")) == 2)
    clf
    clc
    model = m(i,:);
    filename = model;
