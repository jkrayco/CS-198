...
global{
	...
	reflex updateOutput{
		...
		if (mod(cycle,cph) = 0 and hour4 = false){
			save [run, cycle/cph, length(cat), length(sulfide), length(sulfite), length(hydrogen)] to: batchCode+".csv" type:csv;
		}
	}
}
